...
Go CLI tool (`le-go`) that generates Docker Compose configuration files. Built with Cobra + `gopkg.in/yaml.v3`.

## Commands

- `go build -o le-go .` — build binary
...
- `internal/models/compose.model.go` — `Compose`, `Network`, `Volume`, `Service` structs with `Set*` helpers that lazily init maps.
